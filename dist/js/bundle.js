...
;// CONCATENATED MODULE: ./src/js/marked.esm.js
/**
...
;// CONCATENATED MODULE: ./src/js/nodegraph.js
...
async function makeDomNodeContent(graph, node) {
  ...
  if (!nodedata) {
    ...
    const html = parse(nodedata.markdown);
...
;// CONCATENATED MODULE: ./src/js/serialize.js
...
/* harmony default export */ const js_model = (model);
...
function initSidebar() {
  ...
  elemEdgeEditWnd.addEventListener('keyup', (event) => {
    ...
    if (domEdge) {
      if (event.target === document.querySelector('.form-edge-note')) {
        ...
        setEdgeNote(js_model.frame, domEdge, event.target.value);
        updateFrame(js_model.frame);
      }
    }
  });
  ...
  document.querySelector('.btn-new-graph').addEventListener('click', () => {
    if (window.confirm('Starting a new graph will remove the current graph. Are you sure?')) {
      setGraph(js_model.frame, newGraph());
      updateFrame(js_model.frame);
    }
  });
  // * download button
  document.querySelector('.btn-download').addEventListener('click', () => {
    download(js_model.frame.graph);
  });
  // * upload button
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  window.model = js_model;
  // set model.frame as frame DOM element
  js_model.frame = document.querySelector('.frame1');
  // initFrame must be called with frame element before any other calls
  initFrame(js_model.frame);
  // optional. set callback function to be called when selection is changed.
  js_model.frame.callbackSelectionChanged = (domItem) => {
    onSelectionChanged(js_model.frame.graph, domItem);
  };
